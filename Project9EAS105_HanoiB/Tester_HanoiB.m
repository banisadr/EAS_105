...
fid=fopen('WorkingReportHanoiBProjectEAS105F2011.txt','wt');
myfileis=fid;
fprintf(fid, date);
fprintf(fid, '\n \n');
k=clock;
...
fprintf(fid, '\n \n');
fprintf(fid, 'Dear Student:  This is a ''working report'' for the Hanoi Project part B. ');
fprintf(fid, '\n \n');
...
try
        ...
        RSA1=TA2SA(TA1);
        RSA2=TA2SA(TA2);
        if ~isequal(RSA1,[1,2,1,0,1]+1) || ~isequal(RSA2,[2,1,0,2,1]+1)
            testspassed=0;
        end
        ...
        RSA1=TA2SA(TA1);
        RSA2=TA2SA(TA2);
        if ~isequal(RSA1,[1,2,0,0,0]+1) || ~isequal(RSA2,[2,1,0,1,0]+1)
            testspassed=0;
        end
        ...
        RSA1=TA2SA(TA1);
        RSA2=TA2SA(TA2);
        if ~isequal(RSA1,RSA2) || (size(RSA1,2)>1)
            testspassed=0;
        end
